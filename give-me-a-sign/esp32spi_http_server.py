...
class AppServer:
    ...
    def start(self) -> None:
        ...
        for endpoint in self.STORE_ENDPOINTS:
            self._web_app.on(
                "POST",
                f"/{endpoint}",
                lambda environment, key=endpoint: self.store_data(  # pylint: disable=too-many-function-args
        ...
        self._web_app.on(  # pylint: disable=too-many-function-args
            "GET",
            "/info",
            lambda environment: self.info(self, environment),
        )
        self._web_app.on(
            "GET",
            "/data",
            lambda environment: self.data(  # pylint: disable=too-many-function-args
                self, environment
            ),
        )
        self._web_app.on(  # pylint: disable=too-many-function-args
            "POST",
            "/set-time",
            lambda environment: self.set_time(self, environment),
        )
        self._web_app.on(  # pylint: disable=too-many-function-args
            "POST",
            "/image",
            lambda environment: self.image(self, environment),
        )

        http_server.set_interface(self._app.platform.esp)
        # nofmt
    ...
    def info(self, environ) -> list:  # pylint: disable=unused-argument
        ...
        info = {
            ...
            "esp32_firmware": self._app.platform.esp.firmware_version.decode(),
            "wifi": {
                "ssid": self._app.platform.esp.ssid.decode(),
                "bssid": ":".join(
                    "%02x" % b
                    for b in self._app.platform.esp.bssid  # pylint: disable=consider-using-f-string
                ),
                "rssi": self._app.platform.esp.rssi,
                "mac": ":".join(
                    "%02x" % b
                    for b in self._app.platform.esp.MAC_address_actual  # pylint: disable=consider-using-f-string
                ),
                "ip": self._app.platform.esp.pretty_ip(
                    self._app.platform.esp.ip_address
                ),
            },
            "platform": {
